...
// Preload all images as Base64 for instant synchronous PDF generation
console.log('Preloading ' + screens.length + ' screenshots into Base64...');
...
<head>
  <meta charset="UTF-8">
  <title>كتالوج العرض التقديمي الشامل - نظام شركة رواسي عدن</title>
...
<body>
  ...
  <div class="slide-page">
    ...
    <div class="slide-footer">
      ...
      <div>شريحة ${idx + 2} من ${screens.length + 1}</div>
...
async function main() {
  const os = require('os');
  const tmpUserData = path.join(os.tmpdir(), 'edge_pdf_profile_' + Date.now());
  console.log('Launching browser with Puppeteer and isolated profile...');
  ...
  console.log('Setting HTML content directly (Base64 embedded)...');
  await page.setContent(htmlContent, { waitUntil: 'load', timeout: 90000 });

  // Brief pause to ensure fonts are applied
  ...
  console.log('Rendering high-resolution PDF (25 Landscape Slides)...');
  await page.pdf({
    path: destWorkspace,
    format: 'A4',
    landscape: true,
  ...
  console.log('Copying PDF to Desktop...');
  fs.copyFileSync(destWorkspace, destDesktop);

  await browser.close();

  const sz = fs.statSync(destDesktop).size;
  console.log('PDF GENERATED SUCCESSFULLY!');
  console.log('File size: ' + Math.round(sz / 1024) + ' KB');
  console.log('Desktop path: ' + destDesktop);
  console.log('Workspace path: ' + destWorkspace);
}
