...
router.get('/accounts', (req, res) => {
  try {
    const accounts = query('SELECT * FROM accounts ORDER BY code ASC');
...
router.get('/journal-entries', (req, res) => {
  try {
    const entries = query('SELECT * FROM journal_entries ORDER BY date DESC, id DESC');
...
router.post('/journal-entries', (req, res) => {
  try {
    ...
    const countRes = get('SELECT COUNT(*) as cnt FROM journal_entries');
    const entry_no = `JE-${String((countRes.cnt || 0) + 1).padStart(5, '0')}`;
    ...
    for (const line of lines) {
      run(`
        ...
        VALUES (?, ?, ?, ?, ?, ?)
      `, [jeRes.lastInsertRowid, line.account_id, line.project_id || null, Number(line.debit || 0), Number(line.credit || 0), line.notes || '']);
    }

    res.json({ success: true, message: 'تم حفظ القيد اليومي بنجاح', entry_no, id: jeRes.lastInsertRowid });
  } catch (err) {
    res.status(500).json({ success: false, message: 'خطأ في تسجيل القيد', error: err.message });
  }
});

// جلب النثريات والعهد
router.get('/custodies', (req, res) => {
  try {
    const custodies = query('SELECT * FROM custodies ORDER BY date DESC, id DESC');
...
router.get('/cash-movements', (req, res) => {
  try {
    const movements = query('SELECT * FROM cash_movements ORDER BY date DESC, id DESC LIMIT 50');
    const summary = get(`
      SELECT 
